...
async function request() {
  ...
  try {
    ...
    statusEl.textContent = `Formularios: ${r.formularios.length} | Período: ${r.meta.periodo}`;
    return r;
  } catch (e) {
    resEl.textContent = 'No se pudo leer la página. Recargá la Distribución de Tiempo y volvé a intentar.';
...
document.getElementById('json').onclick = async () => {
  const r = await request();
  if (r) download('distribucion_tiempo.json', JSON.stringify(r, null, 2), 'application/json');
};
